...
```{r}
df$month2 <- case_when(df$monthday <=10 ~ 'early', df$monthday <= 20 ~ 'mid', TRUE ~ 'late')
```
...
```{r}
df %>% group_by(month) %>% summarize(median(positiveIncrease))
```

- Calculate the average number of cases (`positiveIncrease`) by `month2`
```{r}
df %>% group_by(month2) %>% summarize(mean(positiveIncrease))
```

- Calculate the median number of cases (`positiveIncrease`) by `weekend`
```{r}
df %>% group_by(weekend) %>% summarize(median(positiveIncrease))
```
...
```{r}
df %>% filter(weekend == 1) %>% group_by(month) %>% summarise(median_positive_cases = median(positiveIncrease))
```


- Calculate the average number of death at different periods of a month (`month2` variable) in Fall 2020
```{r}
df %>% filter(date >= '2020-06-01' & date < '2020-12-01') %>% group_by(month2) %>% summarise(average_num_death= mean(death))
```

- Compare the average number of hospitalizations between weekdays and weekends in Summer 2020
```{r}
df %>% filter(date >= '2020-06-01' & date < '2020-7-01') %>% group_by(weekend) %>% summarise(average_num_hospitalizations = mean(hospitalizedIncrease))
```
...
```{r}
df %>% filter(date >= '2021-01-01' & date <= '2021-12-31', state == 'RI') %>% group_by(weekday) %>% summarise(median_cases = median(positiveIncrease))
```

15. Compare the median cases in Rhode Island in June, July, August and September.
```{r}
df %>% filter(month==6 | month==7 | month==8 | month==9) %>% group_by(month) %>% summarise(median_cases = median(positiveIncrease))
```
...
```{r}
df %>% filter(weekend==1) %>% group_by(month) %>% summarise(average_deaths = mean(death)) %>% arrange(-average_deaths)
```
